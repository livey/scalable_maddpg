...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for agent in world.agents:
            ...
            if agent.adversary== True:
                agent.state.person=0
            else:
                agent.state.person=1
        ####set not collided
        flag=True
        ...
        while flag:
            ...
            for a in ag:
                for aa in ad:
                    ...
                    if dis<1.5*(adver_size+agent_size):
    ...
    def adversary_reward(self, agent, world):
        ...
        if agent.collide:
            collide_time=0
            collide_agent=np.zeros(self.num_agents-1)
            for ag in agents:
                for adv in adversaries:
        ...
        for adv in adversaries:
            if abs(adv.state.p_pos[0]) > self.max_edge or abs(adv.state.p_pos[1]) > self.max_edge:
                rew =np.zeros((self.num_adversaries, 1)) -10
        return rew
    ...
    def observation(self, agent, world):
        global_observation = np.zeros((self.num_agents - 1, 5 * world.dim_p + 1))
        agents = self.good_agents(world)
        adversaries = self.adversaries(world)
        ...
        for j in range(0, self.num_agents - 1):
            ...
            global_observation[j, 4] = theta


            global_observation[j, 5] = global_observation[(j+1)%(self.num_agents-1), 0]
            global_observation[j, 6] = global_observation[(j+1)%(self.num_agents-1), 1]
            global_observation[j, 7] = global_observation[(j+2)%(self.num_agents-1), 0]
            global_observation[j, 8] = global_observation[(j+2)%(self.num_agents-1), 1]
            global_observation[j, 9] = global_observation[(j + 3) % (self.num_agents - 1), 0]
            global_observation[j, 10] = global_observation[(j + 3) % (self.num_agents - 1), 1]
